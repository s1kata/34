        ...
        try {
            if (!$pdo) {
                ...
                if (!$user) {
                    ...
                    try {
                        $pdo->prepare('UPDATE users SET last_login = datetime("now") WHERE id = :id')->execute([':id' => $user['id']]);
...
if (!empty($errors)) {
    header('Location: /frontend/window/login.html?errors=' . urlencode(json_encode($errors)));
    exit;
}
?>